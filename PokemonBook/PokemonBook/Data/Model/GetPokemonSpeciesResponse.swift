...
//  PokemonBook
//
//  Created by HEOGEON on 5/9/25.
//

import Foundation

struct GetPokemonSpeciesResponse: Decodable {
    ...
    func toEntity() -> PokemonSpecies {
        let filterName = names?.first { name in
            return name.language?.name == "ko"
        }
        let pokemonSpices: PokemonSpecies = .init(
            name: filterName?.name ?? ""
        )
        return pokemonSpices
    }
}
